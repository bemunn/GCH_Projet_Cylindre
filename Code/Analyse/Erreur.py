...
ordre_psi =abs(np.log10(erreur_moy_psi[-1])-np.log10(erreur_moy_psi[-2]))/abs(np.log10(N[-1])-np.log10(N[-2])) 
ordre_vr =abs(np.log10(erreur_moy_vr[-1])-np.log10(erreur_moy_vr[-2]))/abs(np.log10(N[-1])-np.log10(N[-2]))
ordre_vtheta =abs(np.log10(erreur_moy_vtheta[-1])-np.log10(erreur_moy_vtheta[-2]))/abs(np.log10(N[-1])-np.log10(N[-2])) 
ordre_cp =abs(np.log10(erreur_moy_cp[-1])-np.log10(erreur_moy_cp[-2]))/abs(np.log10(N[-1])-np.log10(N[-2]))
ordre_cd =abs(np.log10(erreur_moy_cd[-1])-np.log10(erreur_moy_cd[-2]))/abs(np.log10(N[-1])-np.log10(N[-2])) 
ordre_cl =abs(np.log10(erreur_moy_cl[-1])-np.log10(erreur_moy_cl[-2]))/abs(np.log10(N[-1])-np.log10(N[-2])) 


#Plot de l'erreur sur psi
plt.loglog(N,erreur_moy_psi,'-g')
#Ajout d'une pente d'ordre 2 pour visualiser la convergence de l'ordre
plt.loglog(np.array([N[0],N[-1]]), np.array([erreur_moy_psi[-1]/(10 **(-abs(np.log10(N[-1])-np.log10(N[0])) * 2)) ,erreur_moy_psi[-1]]), '--r')
...
plt.ylabel('Erreur')
plt.xlim(N[0],N[-1])  # Fixer les limites en x
plt.grid(which='both')
plt.savefig('ErreurPsiNrNtheta.png', dpi= 1000)
plt.show()

#Plot de  l'erreur sur v_r et v_t
plt.loglog(N,erreur_moy_vr,'-g')
#Ajout d'une pente d'ordre 2 pour visualiser la convergence de l'ordre
plt.loglog(np.array([N[0],N[-1]]), np.array([erreur_moy_vr[-1]/(10 **(-abs(np.log10(N[-1])-np.log10(N[0])) * 2)) ,erreur_moy_vr[-1]]), '--r')
plt.loglog(N,erreur_moy_vtheta,'-b')
#Ajout d'une pente d'ordre 2 pour visualiser la convergence de l'ordre
plt.loglog(np.array([N[0],N[-1]]), np.array([erreur_moy_vtheta[-1]/(10 **(-abs(np.log10(N[-1])-np.log10(N[0])) * 2)) ,erreur_moy_vtheta[-1]]), '--', color = 'orange')
...
plt.loglog(N,erreur_moy_cp,'-g')
#Ajout d'une pente d'ordre 2 pour visualiser la convergence de l'ordre
plt.loglog(np.array([N[0],N[-1]]), np.array([erreur_moy_cp[-1]/(10 **(-abs(np.log10(N[-1])-np.log10(N[0])) * 2)) ,erreur_moy_cp[-1]]), '--r')
...
plt.loglog(N,erreur_moy_cd,'-g')
plt.loglog(N,erreur_moy_cl,'-r')
